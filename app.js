...
let analysisViewType   = 'week';  // 'day' | 'week' | 'month'
let analysisViewOffset = 0;       // 0 = current, negative = past, positive = future
...
function setupAIEventListeners() {
    ...
    document.querySelectorAll('.period-btn').forEach(btn => {
        btn.addEventListener('click', () => setAnalysisViewType(btn.dataset.period));
    });
...
function showAIMode(mode) {
    ...
    switch(mode) {
        case 'analysis':
            ...
            setAnalysisViewType('week'); // Default to this week
            break;
        case 'camera':
...
// ==================== ANALYSIS PERIOD NAVIGATION ====================

function getAnalysisPeriodRange(type, offset) {
    const now = new Date();
    ...
    if (type === 'day') {
        ...
        label = offset === 0
            ? `Today (${wd} ${MNS[d.getMonth()]} ${d.getDate()})`
            : `${wd} ${MNS[d.getMonth()]} ${d.getDate()}, ${d.getFullYear()}`;
        start = d; end = d2;
    } else if (type === 'week') {
        ...
        const rangeStr = mon.getMonth() === sun.getMonth()
            ? `${MNS[mon.getMonth()]} ${mon.getDate()}–${sun.getDate()}`
            : `${MNS[mon.getMonth()]} ${mon.getDate()} – ${MNS[sun.getMonth()]} ${sun.getDate()}`;
        label = offset === 0 ? `This Week (${rangeStr})` : offset === -1 ? `Last Week (${rangeStr})` : `${rangeStr}, ${sun.getFullYear()}`;
        start = mon; end = sun;
    } else {
        const d  = new Date(now.getFullYear(), now.getMonth() + offset, 1);
        const d2 = new Date(d.getFullYear(), d.getMonth() + 1, 0, 23, 59, 59, 999);
        label = offset === 0 ? `This Month (${MNL[d.getMonth()]} ${d.getFullYear()})` : `${MNL[d.getMonth()]} ${d.getFullYear()}`;
        start = d; end = d2;
    }
    return { start, end, label, isFuture: start > now };
}

function setAnalysisViewType(type) {
    analysisViewType   = type;
    analysisViewOffset = 0;
    document.querySelectorAll('.period-btn').forEach(b => b.classList.toggle('active', b.dataset.period === type));
    updateAnalysisPeriodLabel();
    generateCombinedAnalysis();
}

function navigateAnalysisPeriod(delta) {
    analysisViewOffset += delta;
    updateAnalysisPeriodLabel();
    generateCombinedAnalysis();
}

function updateAnalysisPeriodLabel() {
    const { label } = getAnalysisPeriodRange(analysisViewType, analysisViewOffset);
    ...
    setTimeout(async () => {
        ...
        userExercises.forEach(ex => {
            ...
            userData.history.forEach(h => {
                ...
                if (d >= start && d <= end) {
                    periodWorkouts.push({ exercise: ex.name, category: ex.category, muscle: ex.muscle, date: h.date, sets: h.sets });
                }
            });
        });
        ...
        userExercises.forEach(ex => {
            ...
            userData.history.forEach(h => {
                const d = new Date(h.date);
                if (d >= fourWeeksAgo && d < start) {
                    historicWorkouts.push({ exercise: ex.name, category: ex.category, date: h.date, sets: h.sets });
                }
            });
        });

        const hasPastData    = periodWorkouts.length > 0;
        const hasFuturePlans = periodPlans.length > 0;

        if (!hasPastData && !hasFuturePlans) {
            const msg = isFuture
                ? `<h4>📅 ${periodName} — No plans scheduled yet</h4><p>No workouts planned for this period. <a href="#" onclick="document.getElementById('calendarModal').style.display='block';renderCalendar();return false;" style="color:#667eea;">Open the calendar</a> to add plans.</p>`
                : `<h4>📊 ${periodName}</h4><p>No workouts recorded for this period. Get started and I'll provide detailed insights!</p>`;
            ...
            if (dayLogs.length > 0) {
                ...
                sameDayPlans.forEach(p => {
                    ...
                    if (p.startTime && p.endTime) {
                        ...
                        if (mins > 0) { _sdMins = mins; sdDur = ` (~${mins}min)`; }
                    }
                ...
                dayPlans.forEach(p => {
                    ...
                    let _planMins = 60, durationNote = '';
                    if (p.startTime && p.endTime) {
                        const [sh, sm] = p.startTime.split(':').map(Number);
                        const [eh, em] = p.endTime.split(':').map(Number);
                        const mins = (eh * 60 + em) - (sh * 60 + sm);
                        if (mins > 0) { _planMins = mins; durationNote = ` (~${mins}min)`; }
                    }
        ...
        if (userProfile?.goal) {
            const goalMap = {
                'muscle':      'MUSCLE BUILDING — weekly volume per muscle group (10-20 sets), 2× frequency, progressive overload, 8-12 rep ranges.',
                'strength':    'STRENGTH — compound lifts, 3-6 rep range, heavy loads, adequate recovery, progressive overload.',
                'endurance':   'ENDURANCE — training frequency (4-5×/week), high rep ranges (15-20+), sustained cardio integration.',
                'weight_loss': 'WEIGHT LOSS — caloric expenditure through high volume, frequency (4-5×/week), metabolic conditioning.',
                'general':     'GENERAL FITNESS — overall balance, variety, consistency, sustainable habits.'
            };
            goalGuidance = goalMap[userProfile.goal] || goalMap['general'];
        }

        let frequencyNote = '';
        if (analysisViewType !== 'day' && userProfile?.frequency && !isFuture) {
            const target = parseInt(userProfile.frequency);
...
<strong>✅ Top 3 Recommendations</strong>${nutritionInstruction}`;
            ...
            const aiText = await Promise.race([
                callGeminiAI(prompt, null, true, 3200),
            ...
            feedback += `</div></div>`;
            feedback += `<div style="background:#f8f9fa;padding:15px;border-radius:8px;border-left:4px solid #667eea;word-wrap:break-word;overflow-wrap:break-word;">${bodyHTML}</div>`;

        } catch (err) {
            console.error('AI performance analysis failed:', err);
            feedback = `<h4>📊 ${periodName}</h4>`;
            feedback += `<div style="background:#fff3cd;padding:15px;border-radius:8px;border-left:4px solid #ffc107;margin:12px 0;">`;
            feedback += `<p><strong>⚠️ AI unavailable:</strong> ${err.message}</p></div>`;
            if (hasPastData) {
                feedback += `<ul><li>🏋️ ${uniqueExercises} exercises</li><li>💪 ${totalSets} sets</li>`;
                feedback += `<li>⚖️ ${totalVolume.toLocaleString()}kg volume</li><li>📅 ${uniqueDays} training days</li></ul>`;
...
// Compute goal-based personalized weights & reps for a user, given a list of plan exercises.
// Used to fill perUserWeights/perUserReps for invited users before saving to Firebase.
function computePersonalizedWeightsForUser(username, planExercises) {
    const profile = getUserProfile(username);
    ...
    // hypertrophy is the fallback
    const repRangeMin  = isStrength ? 3  : isEndurance ? 12 : 8;
    const repRangeMax  = isStrength ? 6  : isEndurance ? 20 : 12;
    const defaultSets  = isStrength ? 5  : isEndurance ? 3  : 4;

    return planExercises.map(ex => {
        const dbEx    = exercises.find(e => e.name.toLowerCase().trim() === (ex.name || '').toLowerCase().trim());
        const history = dbEx?.users?.[username]?.history || [];
        ...
        if (history.length > 0) {
            ...
            // Check plateau: compare with session from 2+ sessions ago
            const older = history.length >= 3 ? history[history.length - 3] : null;
            const olderAvgW = older ? older.sets.reduce((s, set) => s + set.weight, 0) / older.sets.length : null;
            const plateau = olderAvgW !== null && Math.abs(lastAvgW - olderAvgW) < 0.5;

            let newWeight = lastAvgW;
            let newReps   = Math.round(lastAvgR);
            let newSets   = Math.max(lastSets, defaultSets);

            if (isStrength) {
                newWeight = Math.round(lastAvgW * 1.04 * 2) / 2;
                newReps   = Math.min(repRangeMax, Math.max(repRangeMin, Math.round(lastAvgR)));
            } else if (isEndurance) {
                newWeight = lastAvgW; // keep weight
                newReps   = Math.min(repRangeMax, Math.round(lastAvgR) + 1);
            } else {
                // Hypertrophy — periodize
                if (plateau) {
                    // Plateau: if they've been doing same reps, add weight and reset reps
                    newWeight = Math.round(lastAvgW * 1.05 * 2) / 2;
                    newReps   = repRangeMin;
                } else if (Math.round(lastAvgR) >= repRangeMax) {
                    // At top of rep range — add weight, drop to bottom
                    newWeight = Math.round(lastAvgW * 1.04 * 2) / 2;
                    newReps   = repRangeMin;
                } else {
                    // Within range — add one rep, keep weight
                    newWeight = lastAvgW;
                    newReps   = Math.round(lastAvgR) + 1;
                }
            }

            return { weight: Math.round(newWeight * 2) / 2, reps: newReps, sets: newSets };
        }

        // No exact history — try same category/muscle
        const related = exercises.find(e =>
            e.name !== (ex.name || '') &&
            (e.category === ex.category ||
             (Array.isArray(e.muscle)
                ? e.muscle.some(m => (Array.isArray(ex.muscle) ? ex.muscle : [ex.muscle]).includes(m))
                : e.muscle === ex.muscle)) &&
            (e.users?.[username]?.history?.length || 0) > 0
        );
        if (related) {
            const relLast  = related.users[username].history[related.users[username].history.length - 1];
            const relAvgW  = relLast.sets.reduce((s, set) => s + set.weight, 0) / relLast.sets.length;
            return { weight: Math.round(relAvgW * 0.8 * 2) / 2, reps: Math.round((repRangeMin + repRangeMax) / 2), sets: defaultSets };
        }

        // No data at all — return plan defaults
        return {
            weight: ex.perUserWeights?.[username] ?? ex.plannedWeight ?? 0,
            reps:   ex.perUserReps?.[username]    ?? ex.plannedReps    ?? Math.round((repRangeMin + repRangeMax) / 2),
            sets:   ex.plannedSets ?? defaultSets
        };
    });
}
...
function savePlannedWorkout() {
    ...
    // Deep-clone exercises so we can mutate perUserWeights/perUserReps safely
    const planExercises = JSON.parse(JSON.stringify(currentPlanExercises));

    // Personalize weights & reps for every invited user before saving
    if (invitedUsers.length > 0) {
        invitedUsers.forEach(username => {
            const personalized = computePersonalizedWeightsForUser(username, planExercises);
            planExercises.forEach((ex, i) => {
                if (!ex.perUserWeights) ex.perUserWeights = {};
                if (!ex.perUserReps)    ex.perUserReps    = {};
                // Only fill if AI hasn't already provided a value for this user
                if (ex.perUserWeights[username] === undefined || ex.perUserWeights[username] === null) {
                    ex.perUserWeights[username] = personalized[i].weight;
                }
                if (ex.perUserReps[username] === undefined || ex.perUserReps[username] === null) {
                    ex.perUserReps[username] = personalized[i].reps;
                }
            });
        });
    }
        ...
        const allHistory = withHistory
            .map(ex => {
                ...
                const lastAvgR = lastSession
                    ...
                    : 10;
                const firstAvgW = firstSession
                    ? firstSession.sets.reduce((a, s) => a + s.weight, 0) / firstSession.sets.length
                    : lastAvgW;
                const trend = sessions.length > 1
                    ? (lastAvgW > firstAvgW + 0.5 ? '↑ progressing' : lastAvgW < firstAvgW - 0.5 ? '↓ regressing' : '→ plateau')
                    : '→ first sessions';
...
⚠️ CRITICAL RULES:
1. Use each user's history as the primary data source. Provide genuinely personalized training — not one-size-fits-all.
2. For each exercise per user, choose the progression strategy that best serves their GOAL:
