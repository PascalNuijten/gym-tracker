...
function setupEventListeners() {
    ...
    document.getElementById('testProtectionBtn').addEventListener('click', testDataProtection);
}
    ...
    if (backup) {
        try {
            ...
            if (parsed.exercises && parsed.exercises.length > 0) {
                testResults.push(`<p style="color: green;">✅ PASSED: Backup exists with ${parsed.exercises.length} exercises</p>`);
                testResults.push(`<p style="color: #666; font-size: 0.9em;">Last backup: ${new Date(parsed.timestamp).toLocaleString()}</p>`);
            } else {
                testResults.push('<p style="color: orange;">⚠️ WARNING: Backup exists but has no exercises</p>');
            }
        } catch (e) {
            testResults.push('<p style="color: red;">❌ FAILED: Backup exists but is corrupted</p>');
            allPassed = false;
        }
    } else {
        testResults.push('<p style="color: orange;">⚠️ WARNING: No backup found in localStorage</p>');
        testResults.push('<p style="color: #666; font-size: 0.9em;">Backup will be created on next save</p>');
    }
    
    // TEST 3: Verify current data integrity
    testResults.push('<h3>Test 3: Data Integrity</h3>');
    if (exercises.length > 0) {
        let validCount = 0;
        let invalidCount = 0;
        
        exercises.forEach(ex => {
            if (ex.id && ex.name && ex.users) {
                validCount++;
            } else {
                invalidCount++;
            }
        ...
        if (invalidCount === 0) {
            testResults.push(`<p style="color: green;">✅ PASSED: All ${validCount} exercises have valid structure</p>`);
        } else {
            testResults.push(`<p style="color: red;">❌ FAILED: ${invalidCount} exercises have invalid structure</p>`);
            allPassed = false;
        }
    } else {
        testResults.push('<p style="color: orange;">⚠️ WARNING: No exercises in database to validate</p>');
    }
    
    // TEST 4: Firebase connection
    testResults.push('<h3>Test 4: Firebase Connection</h3>');
    if (database) {
        testResults.push('<p style="color: green;">✅ PASSED: Firebase is connected</p>');
        
        // Check if we can read
        database.ref('exercises').once('value').then(snapshot => {
            const fbData = snapshot.val();
            const fbCount = fbData ? fbData.length : 0;
            
            const statusDiv = document.getElementById('fbStatus');
            if (statusDiv) {
                if (fbCount === originalLength) {
                    statusDiv.innerHTML = `<p style="color: green;">✅ Firebase has ${fbCount} exercises (matches local)</p>`;
                } else {
                    statusDiv.innerHTML = `<p style="color: orange;">⚠️ Firebase has ${fbCount} exercises (local has ${originalLength})</p>`;
                }
            }
        }).catch(e => {
            const statusDiv = document.getElementById('fbStatus');
            if (statusDiv) {
                statusDiv.innerHTML = `<p style="color: red;">❌ Firebase read error: ${e.message}</p>`;
            }
        });
        
        testResults.push('<div id="fbStatus"><p style="color: #666;">Checking Firebase data...</p></div>');
    } else {
        testResults.push('<p style="color: red;">❌ FAILED: Firebase is not connected!</p>');
        allPassed = false;
    }
    
    // TEST 5: Simulate Firebase returning null (the original bug)
    testResults.push('<h3>Test 5: Null Data Handling</h3>');
    testResults.push('<p style="color: #666; font-size: 0.9em;">This simulates the condition that caused the original data loss</p>');
    
    // Check if backup would restore
    if (backup) {
        testResults.push('<p style="color: green;">✅ PASSED: If Firebase returns null, backup would auto-restore</p>');
    } else {
        testResults.push('<p style="color: orange;">⚠️ WARNING: No backup available for auto-restore</p>');
    }
    
    // Summary
    const summary = allPassed ? 
        '<div style="background: #d4edda; border: 1px solid #c3e6cb; padding: 15px; border-radius: 5px; margin-top: 20px;"><h3 style="color: #155724; margin-top: 0;">✅ All Critical Tests Passed!</h3><p style="color: #155724;">Your data is protected against the conditions that caused the previous data loss.</p></div>' :
        '<div style="background: #f8d7da; border: 1px solid #f5c6cb; padding: 15px; border-radius: 5px; margin-top: 20px;"><h3 style="color: #721c24; margin-top: 0;">⚠️ Some Tests Failed</h3><p style="color: #721c24;">Please check the results above and contact support if needed.</p></div>';
    
    testModal.innerHTML = `
        <div class="modal-content" style="max-width: 700px;">
            <span class="close" onclick="this.parentElement.parentElement.remove()">&times;</span>
            <h2>🧪 Data Protection Test Results</h2>
            <p style="color: #666;">Running comprehensive tests to verify all protections are working...</p>
            <hr>
            ${testResults.join('')}
            ${summary}
            <div style="margin-top: 20px; padding: 15px; background: #e7f3ff; border-radius: 5px;">
                ...
                <ul style="margin: 10px 0;">
                    <li>Click <strong>💾 Export</strong> regularly to download backup files</li>
                    <li>Keep backup files in a safe location (Google Drive, Dropbox, etc.)</li>
                    <li>Run this test after adding new exercises to verify backups</li>
                </ul>
            </div>
        </div>
    `;
